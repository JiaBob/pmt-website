<template>
  <header class="flex items-center w-full px-4 py-2 sm:px-4 sm:py-4 fixed top-0 z-10 bg-white bg-opacity-90 text-black transition-all duration-300" :class="bannerIsVisible ? 'h-20 sm:h-24' : 'h-16 sm:h-20 text-sm'">
    <a :href="isHome ? '#' : '/'" class="transform flex justify-center items-center transition-all" :class="bannerIsVisible ? 'h-16 w-auto sm:h-16 sm:w-auto' : 'h-12 w-auto sm:h-12 sm:w-auto'">
      <img class="block sm:hidden h-5/6 -ml-1" :src="$t('logo')" alt="Logo" />
      <img class="hidden sm:block h-full -mr-1" :src="$t('logo')" alt="Logo" />
    </a>
